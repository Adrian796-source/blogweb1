...
public class PermissionControllerIT {
    ...
    @Test
    @DisplayName("GET /api/permissions debería devolver 403 Forbidden para un usuario normal")
    void getPermissions_WhenUserIsNormalUser_ShouldReturnForbidden() throws Exception {
        // Arrange: Crear un usuario con rol USER (sin permisos para leer todos los permisos)
        ...
        // Act 2 & Assert: Intentar acceder a la lista de permisos con el token del usuario normal
    ...
    @Test
    @DisplayName("GET /api/permissions debería devolver 200 OK para un usuario ADMIN")
    void getPermissions_WhenUserIsAdmin_ShouldReturnOk() throws Exception {
        // Arrange: Crear un usuario ADMIN con el permiso 'READ'
        ...
        adminRole.setPermissionsList(Set.of(savedReadPermission));
        ...
        // Act 2 & Assert: Acceder a la lista de permisos con el token de admin
        mockMvc.perform(get("/api/permissions")
                        .header("Authorization", "Bearer " + jwtToken))
                .andExpect(status().isOk());
    }
